...
//public class BPChartRenderer extends XYDifferenceRenderer {
public class BPChartRenderer extends XYLineAndShapeRenderer {
    ...
    public static class State extends XYLineAndShapeRenderer.State {
        ...
        public State(PlotRenderingInfo info) {
            super(info);
            this.lowerCoordinates = new ArrayList<double[]>();
            this.upperCoordinates = new ArrayList<double[]>();
        }
    ...
    private float alpha;
	  public BPChartRenderer () {
		  this(true, true);
	  }
	  
	  public BPChartRenderer (boolean lines, boolean shapes) {
	        super(lines, shapes);
	        ...
	        */
	        ...
	        if (isLinePass(pass)) {
	            ...
	            if (series == 0) {
					...
					double ysys = dataset.getYValue(0, item);
					double ydia = dataset.getYValue(1, item);
					RectangleEdge xAxisLocation = plot.getDomainAxisEdge();
					RectangleEdge yAxisLocation = plot.getRangeAxisEdge();
					double xx = domainAxis
							.valueToJava2D(x, dataArea, xAxisLocation);
					double yysys = rangeAxis.valueToJava2D(ysys, dataArea,
							yAxisLocation);
					double yydia = rangeAxis.valueToJava2D(ydia, dataArea,
							yAxisLocation);
					g2.setColor(new Color(128,128,128));
					g2.drawLine((int) xx, (int) yydia, (int) xx, (int) yysys);
				}
	    ...
	    public boolean equals(Object obj) {
	        ...
	        if (!(obj instanceof BPChartRenderer)) {
	            return false;
	        }
	        BPChartRenderer that = (BPChartRenderer) obj;
